...
class TestProfilerEventsParity(TestCase):
    ...
    def test_profiler_activity_type_parity(self):
        ...
        with TemporaryFileName(mode="w+") as fname:
            ...
            # Chrome trace may have multiple events with same name but different
            # cats (e.g. cpu_op vs kernel). Build a set of (name, cat) pairs.
            json_name_cats = {
                (e["name"], e["cat"])
                for e in j["traceEvents"]
    ...
    def test_profiler_extra_meta_empty_for_regular_ops(self):
        """Verify extra_meta is None for non-NCCL ops."""
        with profile(activities=[ProfilerActivity.CPU]) as prof:
            x = torch.randn(32, 32)
            torch.mm(x, x)

        mm_event = next((e for e in prof.events() if e.name == "aten::mm"), None)
        self.assertIsNotNone(mm_event)
        self.assertIsNone(mm_event.extra_meta)
